...
cat ./db/az_str_10_rule.txt   | head -n 70  | cut -f 1 | grep -vE "^$" > /tmp/str.txt
cat ./db/az_first_10_rule.txt | head -n 93  | cut -f 1 | grep -E "[^'][a-z']" | grep -vE "^$" > /tmp/first.txt
cat ./db/az_last_10_rule.txt  | head -n 118 | cut -f 1 | grep -E "[a-z'][^']" | grep -vE "^$" > /tmp/last.txt
